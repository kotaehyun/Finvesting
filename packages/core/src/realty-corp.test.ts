...
describe("realty corp", () => {
  it("4급·연예인은 넣지 않고 법인은 종부세 집계로 넣는다", () => {
    expect(REALTY_CORP_CHANNELS.find((c: any) => c.id === "grade4")?.put).toBe("never");
    expect(REALTY_CORP_CHANNELS.find((c: any) => c.id === "celeb")?.put).toBe("never");
    expect(REALTY_CORP_CHANNELS.find((c: any) => c.id === "corp")?.put).toBe("in");
