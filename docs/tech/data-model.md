...
| 테이블 | 역할 |
|---|---|
| `accounts` | 계좌. `name` 통장 이름, `institution` 은행/증권사. `type`(checking/savings/installment/brokerage/crypto/card/cash/pension/loan)이 자산 배분 분류 기준. `balance`는 현재 잔액. `/profile` 통장내역에서 은행·통장을 구분해 보고 `accounts.update`로 수정 |
...
| `trades` | 체결 원장. 보유 수량·평단·실현손익은 저장하지 않고 `core/portfolio.ts`로 계산. **계좌별**로 포지션을 나눔(같은 종목이 증권사마다 있으면 행이 둘). 자산군·계좌 소계는 `core/holdings-summary.ts`. 웹 `/holdings`에서 입력, `/profile` 09 투자내역에서 전체·계좌·주식/ETF/채권/펀드/코인을 조회. 평가손익은 종목 최근 `market_quotes.close` (해외는 `USDKRW`가 있으면 원화 환산). 대시보드 투자 자산은 증권·코인·연금 계좌 잔액과 보유 평가액 중 큰 쪽 |
