...
- 웹 UI: `/chat` **투자 봇AI**. **대화형 배치**(빈 화면 인사·아래 입력칸. 자산 홈 파랑 대신 청록 팔레트. ADR 0048). 대화 목록(`apps/web/src/app/chat/starters.ts`)은 자산·현금흐름·보유·시장·판단 + 재무제표/감사/펀더멘털/투자성향 + 칼럼·애널리스트. 처음엔 그룹당 1개 제안, 「다른 질문」으로 나머지. 대화가 시작되면 제안은 접고 「새 대화」만. ChatGPT 복제가 아님. 대화는 이 탭에만 있고 저장하지 않음. `/profile` 11 보험내역 **AI 추천**, 10 투자성향 **AI 추천**. `/statements` 「읽어주기」는 `statements.explain`. `/fundamentals`에는 AI 분석 필을 두지 않는다 (ADR 0028).
- 챗 컨텍스트: 최근 뉴스 15건(오피니언 소스 제외) + 오피니언·칼럼 8건 + 세계 지수 종가(Yahoo 수집분) + 보유 종목 Yahoo 펀더멘털(status=ok일 때만) + 투자성향·조언
...
현재 TypeScript/tRPC 백엔드와 Ollama를 유지하고, 데이터 안정화 이후 질문 기반 RAG를 단계적으로 추가한다. Java·Python·특정 AI 프레임워크는 필수가 아니다. 별도 서버는 구체적인 실행 환경·운영 요구가 있을 때 검토한다. 기술별 역할·검색 목표 흐름·이전 판단 기준은 [ADR 0049](./decisions/0049-backend-ai-evolution.md)에 기록했다. 후속 기능은 계획이며 구현 완료 상태가 아니다.
