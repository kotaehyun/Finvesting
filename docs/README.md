...
1. 새 세션은 `prompts/00-bootstrap.md`를 붙여 넣어 시작하고, `prompts/90-session-end.md`로 끝낸다.
2. 작업 시작 전 `progress/README.md`, `tech/overview.md`, `tech/architecture.md`를 읽는다. 건드리는 영역의 tech 문서도 읽는다.
3. 문서에 없는 것은 **추측하지 말고** 코드를 확인하거나 사용자에게 묻는다.
4. 작업 후 반드시 `progress/README.md`에 날짜·한 일·다음 할 일을 추가한다.
5. 구조/스키마/소스/AI/설정을 바꿨으면 해당 `tech/` 문서를 갱신하고, 중요한 기술 선택은 `tech/djecisions/`에 ADR 한 장 추가.
6. 오류를 만나면 `logs/`에, 코드를 리뷰하면 `review/`에 기록한다.
7. 세션마다 `verification/YYYY-MM-DD-<모델>.md`에 **실제로 실행해 확인한 것**과 **못 한 것**을 남기고, `verification/README.md`의 검증 현황 표를 갱신한다. 실행 안 해본 것을 검증됐다고 쓰지 않는다.
8. 문서는 한국어, 코드 식별자는 영어 그대로. 핵심 문서는 `*.en.md` 영문 미러가 있으니 한국어를 고치면 같은 세션에서 영문도 갱신한다.
