...
5. 패키지 의존 규칙(`docs/tech/architecture.md`)을 지킨다: core는 순수, web/mobile은 DB 직접 접근 금지.
6. 금액은 `numeric` 문자열 ↔ `Number()` 변환. 뉴스 본문은 저장하지 않는다.
7. 언어: 문서·주석은 한국어, 식별자는 영어. 패키지 매니저는 pnpm만.
